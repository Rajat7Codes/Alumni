package com.example.alumniadmin.serviceIMPL;
...
public class PostServiceIMPL implements PostService {
    ...
    @Override
    public void deleteAdminPost(@NonNull CallBack<Boolean> finishedCallback, Post post) {
        databaseReference.child("admins").child( "posts").child( post.getId())
                .removeValue()
                .addOnCompleteListener(
                        task -> finishedCallback.callback( task.isSuccessful()));
    }
    ...
    @Override
    public void updateAdminPost(@NonNull CallBack<Boolean> finishedCallback, Post post) {
        String key = post.getId();
        Map<String, Object> postValues = post.toMap();

        Map<String, Object> childUpdates = new HashMap<>();
        childUpdates.put("/admins/posts/" + key, postValues);
